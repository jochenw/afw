...
public class Rcm extends AbstractInitializable {
	...
	private List<RmTargetLifecyclePlugin> targetLifecyclePlugins;
	...
	public static RcmBuilder builder() {
		return new RcmBuilder();
	}
	...
	public void init(ComponentFactory pComponentFactory) {
		...
		List<RmTargetLifecyclePlugin> targetList = componentFactory.requireList(RmTargetLifecyclePlugin.class);
		targetLifecyclePlugins = targetList;
	}
	...
	public void run() {
		final List<RmTargetLifecyclePlugin> targetPluginList = targetLifecyclePlugins;
		final List<Context> contextList = new ArrayList<>(targetPluginList.size());
		for (RmTargetLifecyclePlugin tlp : targetPluginList) {
			final Context ctx = tlp.start();
			contextList.add(ctx);
			if (contextMap.put(ctx.getId(), ctx) != null) {
				throw new IllegalStateException("Duplicate ID for target lifecycle plugin:" + ctx.getId());
			}
		}
		Throwable th = null;
		try {
			...
			for (int i = 0;  i < targetPluginList.size();  i++) {
				try {
					targetPluginList.get(i).commit(contextList.get(i));
				} catch (Throwable t) {
					if (th == null) {
						th = t;
					}
		...
		if (th != null) {
			for (int i = targetPluginList.size()-1;  i >= 0;  i--) {
				try {
					targetPluginList.get(i).rollback(contextList.get(i));
				} catch (Throwable t) {
					// Ignore this
				}
			}
	...
	private void install(final List<DetectedResourceInfo> resources) {
		for (DetectedResourceInfo dri : resources) {
			final RmResourcePlugin plugin = dri.getPlugin();
			final String id = plugin.getContextId();
			if (id == null) {
				dri.setContext(null);
			} else {
				final Context ctx = contextMap.get(id);
				if (ctx == null) {
					throw new IllegalStateException("Context not available for plugin " + plugin.getClass().getName() + ", and context ID " + id);
				}
				dri.setContext(ctx);
			}
	...
	protected List<DetectedResourceInfo> findResourceRefs() {
		...
		for (final RmResourceRefRepository rmrrr : resourceRepositories) {
			for (final RmResourceRef rrr : rmrrr.getResources(logger)) {
				...
				RmResourceInfo rri;
				if (req.getResourceRef() instanceof RmResourceInfo) {
					rri = (RmResourceInfo) req.getResourceRef();
				} else {
					rri = null;
