...
public class PeekableInputStreamTest {
	/** Test case for reading a byte array.
		...
		while (inputOffset < pInput.length) {
			...
			switch(action) {
			  case 0:  // Read a few bytes.
			  {
				...
				for (int i = 0;  i < res;  i++) {
					assertEquals(pInput[inputOffset++], buffer[i], "Seed=" + pSeed + ", pos=" + inputOffset);
				}
				break;
			  }
			  case 1:  // Read a single byte.
			  {
				...
				assertEquals((int) pInput[inputOffset++], res, "Seed=" + pSeed + ", pos=" + inputOffset);
				break;
			  }
